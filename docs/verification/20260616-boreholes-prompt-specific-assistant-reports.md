...
Source-control note: the former `database/010_configure_boreholes_app_pages.sql` mixed database package source with APEX component delivery. The package source is retained in `database/010_create_boreholes_page_api.sql`; page/component state is represented by the application export.
