...
**Current status: all four package bodies are installed, VALID, error-free and verified against the approved complete source; specifications are unchanged.** Page4/page5/page10030 components, native Explorer, runtime acceptance and post-APEX exports remain pending. See [the deployment checkpoint](20260917-boreholes-review-implementation.md#current-deployment-checkpoint--17-september-2026). GEO released all browser ownership to AI Hub root under v12; await the next explicit GEO release before any browser action.
...
**Next delivery delta:** the first four-body deployment below is complete. The subsequent Unicode acceptance probe requires only one additional `GS_BOREHOLE_AGENT_API` body update: extracted statement 31,399 UTF-8 bytes, SHA-256 `bbee732020f37658e703d3b0f2bffce1f8c908bdc61152488f1166e3f5ced0a0`. It passed the six-case/25-assertion Unicode probe and 79 source regressions with zero provider attempts. Reinspect its exact bytes and register a fresh upload copy; the original agent upload and historical hash below do not contain this repair. Preserve the current after-package checkpoint as the pre-repair source, verify that live source still matches it, then replace only this body and capture a new source checkpoint.
...
| Installed bodies | All four installed and VALID/errors0; exact full-source comparisons passed and all specs unchanged. |
...
| p4/p5/p10030 changes, native p2 and app104/app105 runtime checks | Component delivery and browser acceptance remain pending. |
