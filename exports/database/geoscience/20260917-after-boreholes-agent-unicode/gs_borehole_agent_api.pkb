CREATE OR REPLACE package body gs_borehole_agent_api as
  ...
  begin
    if p_text is not null then
      -- CLOB amounts use UCS2 units, including both halves of a surrogate pair.
      dbms_lob.writeappend(p_clob, length2(p_text), p_text);
    end if;
  end;
  ...
  begin
    -- Bound reads to 4000 UCS2 units; allow their multibyte VARCHAR2 representation.
    while l_offset <= nvl(dbms_lob.getlength(p_value), 0) loop
      ...
      -- SUBSTR may adjust a read at a surrogate boundary: advance by actual units.
