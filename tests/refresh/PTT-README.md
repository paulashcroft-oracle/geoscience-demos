...
`installed-api-acceptance.sql` is a separate integration test, not an installer or a PTT fixture. It is **compile-verified but not executed**. Before running it, root must freshly verify its own SQL Commands page, baseline/checkpoint, exclusive execution window, and installed `GS_BOREHOLE_REFRESH_API` body against extracted SHA256 `39c352c0728239794291865c1e844d4d31747a857a70eab7e6056d6ff7a44b42` (20,445 UTF-8 bytes), using the observed SQL Scripts mapping below. The block does not perform that source comparison. Its runtime guard requires database unique name `tcelkxkd`, current schema/workspace `GEOSCIENCE`, session user `ORDS_PLSQL_GATEWAY` and APEX user `CODEX`; these identities alone do not prove the active page is SQL Commands.

On 17 September 2026, root reported successful compilation of the exact current fixture inside an **uncalled local procedure**, through saved SQLcl connection `aidemodb`; fresh outer guards verified CODEX / `tcelkxkd` / no APEX session, and SQLcl exited0. Only the terminal slash was removed from the fixture in memory, with zero package-qualification substitutions. Fixture invocations, HTTP/model calls, business DML and APEX session creation were all zero. The unchanged file SHA256 is `c16d4d8e4f2b6b38c3401f32ee569cc494c534a338cfdb3801cda1b5ea87a1dc`. This proves compilation only: the fixture's own execution guards, installed public wrapper, HTTP request and rollback assertions still need the authorized owning SQL Workshop execution.
...
The verified SQL Scripts `USER_SOURCE` comparison is **458 rows / 20,427 UTF-8 bytes / SHA256 `b539e67439d2b7f8b82fb24a999bb4798d7937fe802ce86172efc886ad619cac`**: exclude the `CREATE OR REPLACE ` prefix, replace each canonical CR with an ordinary space while retaining LF, map displayed NBSP to ordinary space, and trim only final whitespace, preserving complete ordered lines. This supersedes the earlier prepared CR-removal digest. Failure diagnostics additionally retain only the API's allowlisted error classification; no raw message is emitted. Once rollback parity passes, LOB cleanup failures are reported separately without comparing freed baseline locators.
